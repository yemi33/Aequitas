...
def run_aequitas(dataset: Dataset, perturbation_unit, cleaned_csv_dir, pkl_dir, improved_pkl_dir, retrain_csv_dir, plot_dir, \
    ...
    for i in range(num_of_sens_param):
        dataset.update_sensitive_parameter(dataset.sensitive_param_idx_list[i])
        run_aequitas_once(dataset, perturbation_unit, cleaned_csv_dir, pkl_dir, improved_pkl_dir, \
